...
def slow_attn(query, key, value, scale: float, attn_mask=None, dropout_p=0.0):
    ...
    if dropout_p > 0:
        F.dropout(attn_weights, p=dropout_p, inplace=True)

    log_attn_weights = torch.log2(attn_weights + 1e-9)
    entropy_term = attn_weights * log_attn_weights
    attention_entropy = -torch.sum(entropy_term, dim=-1)
    return attn_weights @ value, attention_entropy
