...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="HPSv3 evaluation (Original-only or with RL/Fastvar)."
    )
    parser.add_argument(
        "--original",
        "--origianl",
        ...
        dest="original",
        required=True,
        help="Folder with original images and meta_data.json",
    )
    parser.add_argument(
        "--fastvar",
        "--f",
        dest="fastvar",
        default=None,
        help="Folder with fastvar images (optional)",
    )
    parser.add_argument("--RL", "--rl", dest="RL", default=None, help="Folder with RL images (optional)")
...
def main() -> None:
    ...
    folders = [args.original]
    if args.RL:
        folders.append(args.RL)
    if args.fastvar:
        folders.append(args.fastvar)
    ...
    rl_files = set(list_images(args.RL)) if args.RL else None
    fast_files = set(list_images(args.fastvar)) if args.fastvar else None
    common_files = orig_files
    if rl_files is not None:
        common_files = common_files & rl_files
    if fast_files is not None:
        common_files = common_files & fast_files
    common_files = sorted(common_files)
    if not common_files:
        raise RuntimeError("No matching filenames across the provided folders.")
    ...
    for fname in tqdm(common_files, desc="Scoring with HPSv3"):
        ...
        if args.RL:
            paths["Score_RL"] = os.path.join(args.RL, fname)
        if args.fastvar:
            paths["Score_Fastvar"] = os.path.join(args.fastvar, fname)
    ...
    score_columns = ["Score_Original"]
    if args.fastvar:
        score_columns.append("Score_Fastvar")
    if args.RL:
        score_columns.append("Score_RL")
    df = pd.DataFrame(rows, columns=["Filename", "Text_Prompt", *score_columns])
    df.to_csv(output_csv, index=False)
    print(f"Saved results to {os.path.abspath(output_csv)}")
    averages = df[score_columns].mean().to_dict()
    for key in score_columns:
        print(f"{key} mean: {averages.get(key):.6f}")
