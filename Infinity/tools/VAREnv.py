...
class VAREnv(gym.Env):
    ...
    def _ensure_golden_images(self, g_seed=42, cfg_list=None, tau_list=None,
        ...
        # Fast path: all scales cached → just load into memory and return.
        if all_cached:
            with torch.no_grad():
                for si in range(num_scales):
                    img_path = os.path.join(prompt_dir, f"scale_{si}_golden.png")
    ...
    def step(
        ...
        save_dir: str = "/home/remote/LDAP/r14_jameschen-1000043/FastVAR/Infinity/training_tmp_results",
        ):
        ...
        speed_score = prune_ratio * weight_scale[current_scale] * (1.5 if prune_ratio == 1.0 else 1.0)
        ...
        full_obs = torch.cat([codes_resized, scale_plane], dim=0)  # [C+1, H, W]
